...
def arg_parse():
    ...
    parser.add_argument('-l', '--log', dest='log', type=str,
                        metavar='FILE', default="run_data.log", help='log file name')
    parser.add_argument('--input_dir', dest='input_dir', type=str,
                        metavar='DIR', default="data", help='input dir')
    parser.add_argument('--resultq', dest='resultq', type=str,
                        metavar='FILE', default="quicksort.xlsx", help='excel file name')
    parser.add_argument('--exe', dest='exe', type=str,
                        metavar='FILE', default="hw5_q.exe", help='exe file name')
    parser.add_argument('--resultpns', dest='resultpns', type=str,
                        metavar='FILE', default="pns.xlsx", help='excel file name')
...
if __name__ == '__main__':
    ...
    if not os.path.exists(report_path):
        ...
        exit()
    report_fileq = os.path.join(report_path, args.resultq)
            ...
            for i in range(repeat_times):
                ...
                for line in io.StringIO(stdout):
                    ...
                    if match_int or match_float or match_exp:
                        ...
                        scores.to_excel(report_fileq)
                        break
                if no_result:
                    logger.error("{}th {} fail. Output:{}".format(
                        i, f, stdout.encode("utf8").decode("utf8")))
                    break
